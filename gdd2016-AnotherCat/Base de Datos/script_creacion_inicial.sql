...
--		Top 5 de las especialidades de médicos con más bonos de consultas utilizados
 ...
 if OBJECT_ID(N'NOT_NULL.Get_Turnos_Prof_ReservadosTodos') is not null
	drop proc NOT_NULL.Get_Turnos_Prof_ReservadosTodos
GO
  create procedure NOT_NULL.Get_Turnos_Prof_ReservadosTodos(@medxespid int, @fecha datetime)
 as
	begin
		select * from turno where turno_medico_especialidad_id = @medxespid and YEAR(turno_fecha) = YEAR(@fecha)
